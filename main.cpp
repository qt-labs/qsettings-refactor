...
static void printSettings(const DConfSettings *settings, const QString &prefix)
{
    ...
    if (!keys.isEmpty()) {
        ...
        Q_FOREACH (const QString &key, keys) {
            QVariant value;
            settings->get(prefix + key, &value);
            values.insert(key, value);
        }
        qDebug() << prefix << ":" << values;
    }

    const QStringList groups = settings->children(prefix, Settings::ChildGroups);
    Q_FOREACH (const QString &group, groups)
        printSettings(settings, prefix + group);
}
